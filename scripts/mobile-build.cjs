...
const NATIVE_REV = 13; // v0.43.0 — NamaNative.setImmersive/setOrientation land in the native layer (the WebView fullscreen fix: system-bar hiding + real orientation lock) AND PlayerActivity gets the ⋮ overflow controller → APK required for every user on ≤ rev 12. The v0.43.0 web bundle degrades gracefully on old APKs (virtual fullscreen without system-bar hiding).
