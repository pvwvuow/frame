    ...
    <div className={`flex flex-col ${compact ? "gap-1" : "gap-2"}`}>
      {!compact && (
        <p className="text-xs font-bold text-zinc-400">
          {tr("title.yourRating")}{score ? <span className="ms-2 rounded-md bg-amber-400/15 px-1.5 py-0.5 text-amber-400">{fa(score)}/۱۰</span> : null}
        </p>
      )}
      <div className="flex items-center gap-0.5" dir="ltr" onMouseLeave={() => setHover(0)} role="radiogroup" aria-label={tr("title.yourRating")}>
