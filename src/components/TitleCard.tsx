...
export default function TitleCard({
  ...
  return (
    <div className={`group relative block shrink-0 ${w} snap-start`}>
      ...
      >
        <div className="relative aspect-[2/3] overflow-hidden rounded-xl bg-ink-700 ring-1 ring-white/5 transition-all duration-300 group-hover:-translate-y-1.5 group-hover:shadow-[0_20px_50px_rgba(0,0,0,0.6),0_0_0_1px_rgba(255,255,255,0.14)] group-focus-visible:ring-2 group-focus-visible:ring-brand">
          ...
          <div className="absolute end-2 top-2 flex items-center gap-1">
            {fav && (
              <span className="grid h-5 w-5 place-items-center rounded-md bg-rose-600 text-white shadow" aria-label="در علاقه‌مندی‌ها">
            ...
            {saved && !fav && (
              <span className="grid h-5 w-5 place-items-center rounded-md bg-brand text-white shadow" aria-label="در لیست شما">
      ...
      <div className="absolute bottom-[54px] start-2 flex translate-y-2 items-center gap-1.5 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:translate-y-0 group-focus-within:opacity-100">
        <FavoriteButton titleId={t.id} name={t.title} variant="mini" />
        <WatchlistButton titleId={t.id} name={t.title} variant="mini" />
      </div>
