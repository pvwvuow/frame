 ...
 * infinite-skeleton failure mode with an honest Persian message + a retry. */
...
export default function LoadErrorCard({ onRetry }: { onRetry: () => void }) {
  return (
    ...
    >
      ...
      <p className="text-sm font-black text-white">بارگیری اطلاعات ناموفق بود</p>
      <p className="text-xs leading-6 text-zinc-500">اتصال اینترنت را بررسی کنید و دوباره تلاش کنید.</p>
      <button
        type="button"
        onClick={onRetry}
