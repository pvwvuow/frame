...
> به‌روزرسانی خودکار: نسخه‌ی دسکتاپ خودش نسخه‌ی جدید را (دلتای کم‌حجم) دانلود و نصب می‌کند. در اندروید، به‌روزرسانی‌های کوچک «درجا» و بی‌صدا نصب می‌شوند؛ برای نسخه‌های بزرگ، لینک دانلود در مرورگر باز می‌شود و مثل بار اول نصب می‌کنید — تنظیمات و سابقه‌ی شما حفظ می‌ماند.

### 🤖 نصب اندروید — درباره‌ی هشدارها

فریم در Google Play منتشر نمی‌شود و مستقیم از GitHub دانلود می‌شود. اندروید برای هر برنامه‌ای خارج از استور هشدار می‌دهد؛ این به معنی وجود ویروس **نیست**:

1. فایل `Frame-<نسخه>-android.apk` را از [Releases](https://github.com/pvwvuow/frame/releases/latest) بگیرید.
2. روی فایل بزنید → اگر پیام «اجازه نصب از این منبع» آمد → **تنظیمات** → اجازه بدهید → برگردید.
3. اگر پیام Google Play Protect آمد → **More details** → **Install anyway**.
4. (اختیاری) اصالت فایل: SHA-256 فایل APK در یادداشت‌های هر Release نوشته شده است (با `apksigner verify --print-certs` یا اپ‌هایی مثل App Manager قابل مقایسه است).
